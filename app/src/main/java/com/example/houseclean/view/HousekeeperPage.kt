...
class HousekeeperPage : AppCompatActivity() {
    ...
    private lateinit var navTask: LinearLayout
    private lateinit var navAttendance: LinearLayout
    private lateinit var navAlert: LinearLayout

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_housekeeper_page)
        ...
        navTask = findViewById(R.id.nav_task_container)
        navAttendance = findViewById(R.id.nav_attendance_container)
    ...
    private fun setupProfileIcon() {
        ...
        userRepository.getUserData(uid).addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                user?.let {
                    val initials = if (it.firstName.isNotEmpty() && it.lastName.isNotEmpty()) {
                        "${it.firstName.take(1)}${it.lastName.take(1)}".uppercase()
                    } else "HK"
    ...
    private fun setupNavigation() {
        ...
        navTask.setOnClickListener { selectTab(R.id.nav_task_container) }
        navAttendance.setOnClickListener { selectTab(R.id.nav_attendance_container) }
    ...
    private fun selectTab(containerId: Int, shouldNavigate: Boolean = true) {
        ...
        if (shouldNavigate) {
            when (containerId) {
                R.id.nav_home_container -> replaceFragment(HousekeeperHomeFragment())
                R.id.nav_task_container -> replaceFragment(TasksFragment())
                R.id.nav_attendance_container -> replaceFragment(AttendanceFragment())
    ...
    private fun resetAllTabs() {
        val navIds = listOf(R.id.nav_home_container, R.id.nav_task_container, R.id.nav_attendance_container, R.id.nav_alert_container)
        for (id in navIds) {
